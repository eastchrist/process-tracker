...
export default class extends Vue {
  ...
  private top = 0
  private left = 0
  private selectedTag: AITagView = {}
  private affixTags: AITagView[] = []
  ...
  private isActive(route: AITagView) {
    return route.path === this.$route.path
  }

  private isAffix(tag: AITagView) {
  ...
  private moveToCurrentTag() {
    ...
    this.$nextTick(() => {
      for (const tag of tags) {
        if ((tag.to as AITagView).path === this.$route.path) {
          (this.$refs.scrollPane as ScrollPane).moveToTarget(tag as any)
          // When query is different then update
          if ((tag.to as AITagView).fullPath !== this.$route.fullPath) {
  ...
  private refreshSelectedTag(view: AITagView) {
  ...
  private closeSelectedTag(view: AITagView) {
  ...
  private closeAllTags(view: AITagView) {
  ...
  private toLastView(visitedViews: AITagView[], view: AITagView) {
  ...
  private openMenu(tag: AITagView, e: MouseEvent) {
...
<style lang="scss" scoped>
  .tags-view-container {
    ...
    width: 100%;
    //background: #ff5186;
    background: $tagviewBackgroundColor;
