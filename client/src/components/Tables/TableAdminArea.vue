<template>
    <div class="TableAdminArea-container">
        <div class="wrapper">
            <div class="header">
                <h3>{{ $t('tables.area.title') }}</h3>
            </div>
            <div class="filter">
                <el-input v-model="listQuery.name" :placeholder="$t('tables.area.champs.champs4')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
                <el-select v-model="listQuery.idFactory" :placeholder="$t('tables.area.champs.champs7')" clearable class="filter-item" style="width: 200px">
            ...
            <div class="extra">
                <ExportToExcel className="ExportToExcel" TableName="Area" :TableList=list :defaultExcelHeater="listExcel" ></ExportToExcel>
                <ImportFromExcel className="ImportFromExcel" TableName="Area" :TableList=list :defaultExcelHeater="listExcel" :on-success="handleSuccess" :before-upload="beforeUpload"></ImportFromExcel>
                <UpdateFromNode className="UpdateFromNode" TableName="Area" @UpdateFromNode="getList"></UpdateFromNode>
            </div>
            <div class="table">
                <el-table ref="draggableTable" row-key="position" :key="tableKey" stripe v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
                    ...
                    <el-table-column :label="$t('tables.area.champs.champs4')" min-width="250px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.area.champs.champs5')" min-width="150px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name1 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.area.champs.champs6')" min-width="150px">
                        <template slot-scope="{row}">
                            <span >{{ row.menu }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.area.champs.champs7')" min-width="150px">
                        <template slot-scope="{row}">
                            <span >{{ row.factory.name }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.area.champs.champs8')" align="center" width="170" class-name="fixed-width">
            ...
            <div class="treeview"><TreeAdminNodes class-name="TreeNodes_Area" :title="$t('tree.areaTree')" Find1="Factory" Find2="Area" Find3=""></TreeAdminNodes></div>
...
<script lang="ts">
    ...
    import { getDBAreas, updateDBAreas, updateDBArea, deleteDBArea, defaultAreaData, defaultAreaRules, defaultAreaQuery, defaultAreaExcelHeater } from '@/api/areas'
    ...
    export default class Dashboard extends Vue {
        ...
        private listExcel = defaultAreaExcelHeater
