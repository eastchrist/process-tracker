<template>
    <div class="table-container">
        <div class="wrapper">
            <div class="filter">
                <el-input v-model="listQuery.name" :placeholder="$t('tables.equip.placeholder.placeholder1')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
                <el-select v-model="listQuery.idPlc" :placeholder="$t('tables.equip.placeholder.placeholder2')" clearable class="filter-item" style="width: 200px">
                    <el-option v-for="item in TypeOptionPlc" :key="item.id" :label="item.name" :value="item.id"/>
                </el-select>
                <el-select v-model="listQuery.idArea" :placeholder="$t('tables.equip.placeholder.placeholder3')" clearable class="filter-item" style="width: 200px">
                    <el-option v-for="item in TypeOptionArea" :key="item.id" :label="item.name" :value="item.id"/>
                </el-select>
                <el-select v-model="listQuery.idFactory" :placeholder="$t('tables.equip.placeholder.placeholder4')" clearable class="filter-item" style="width: 200px">
            ...
            <div class="extra">
                ...
                <ImportFromExcel class="ImportFromExcel"  className="ImportFromExcel" TableName="Equip" :TableList=list :defaultExcelHeater="listExcel" :on-success="handleSuccess" :before-upload="beforeUpload"></ImportFromExcel>
            ...
            <div class="table">
                ...
                <el-table ref="draggableTable" row-key="position" :key="tableKey" stripe v-loading="listLoading" :data="list" border fit highlight-current-row @sort-change="sortChange" style="width: 100%;" :header-cell-style="getDesignElementUiHeaderStyle" :cell-style="getDesignElementUiCellsStyle">
                    <!-- titre table -->
                    <el-table-column :label="$t('tables.equip.title')">
                        ...
                        <el-table-column :label="$t('tables.equip.champs.champs4')" :min-width="designTable.columns[1].width">
                            <template slot-scope="{row}">
                                <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs5')" :min-width="designTable.columns[2].width">
                            <template slot-scope="{row}">
                                <span >{{ row.name1 }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs6')" :min-width="designTable.columns[3].width">
                            <template slot-scope="{row}">
                                <span >{{ row.menu }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs7')" :min-width="designTable.columns[4].width">
                            <template slot-scope="{row}">
                                <span >{{ row.plc.name }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs8')" :min-width="designTable.columns[5].width">
                            <template slot-scope="{row}">
                                <span >{{ row.plc.area.name }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs9')" :min-width="designTable.columns[6].width">
                            <template slot-scope="{row}">
                                <span >{{ row.plc.area.factory.name }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs10')" :min-width="designTable.columns[7].width">
                            <template slot-scope="{row}">
                                <span >{{ row.idEquipDef }}</span>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('tables.equip.champs.champs11')" :min-width="designTable.columns[8].width">
...
<script lang="ts">
    ...
    export default class Dashboard extends Vue {
        private designTable = defaultEquipDesignTable
