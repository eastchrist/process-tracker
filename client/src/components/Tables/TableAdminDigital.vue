<template>
    <div class="TableAdminDigital-container">
        <div class="wrapper">
            <div class="header">
                <h3>{{ $t('tables.digital.title') }}</h3>
            </div>
            <div class="filter">
                <el-input v-model="listQuery.name" :placeholder="$t('tables.digital.champs.champs4')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
                <el-select v-model="listQuery.idFactory" :placeholder="$t('tables.digital.champs.champs5')" clearable class="filter-item" style="width: 200px">
            ...
            <div class="extra">
                <ExportToExcel className="ExportToExcel" TableName="Digital" :TableList=list :defaultExcelHeater="listExcel" ></ExportToExcel>
                <ImportFromExcel className="ImportFromExcel" TableName="Digital" :TableList=list :defaultExcelHeater="listExcel" :on-success="handleSuccess" :before-upload="beforeUpload"></ImportFromExcel>
                <UpdateFromNode className="UpdateFromNode" TableName="Digital" @UpdateFromNode="getList"></UpdateFromNode>
            </div>
            <div class="table">
                <el-table ref="draggableTable" row-key="position" :key="tableKey" stripe v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
                    ...
                    <el-table-column :label="$t('tables.digital.champs.champs3')" min-width="140px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.digital.champs.champs4')" min-width="140px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name1 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.digital.champs.champs5')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.tag }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.digital.champs.champs6')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.address }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.digital.champs.champs7')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.idEquip }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.digital.champs.champs8')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.idTank }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.digital.champs.champs9')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.indexCreation }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.digital.champs.champs11')" align="center" width="170" class-name="fixed-width">
