<template>
    <div class="TableAdminMeasure-container">
        <div class="wrapper">
            <div class="header">
                <h3>{{ $t('tables.measure.title') }}</h3>
            </div>
            <div class="filter">
                <el-input v-model="listQuery.name" :placeholder="$t('tables.measure.champs.champs4')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
                <el-select v-model="listQuery.idFactory" :placeholder="$t('tables.measure.champs.champs5')" clearable class="filter-item" style="width: 200px">
            ...
            <div class="extra">
                <ExportToExcel className="ExportToExcel" TableName="Measure" :TableList=list :defaultExcelHeater="listExcel" ></ExportToExcel>
                <ImportFromExcel className="ImportFromExcel" TableName="Measure" :TableList=list :defaultExcelHeater="listExcel" :on-success="handleSuccess" :before-upload="beforeUpload"></ImportFromExcel>
            </div>
            <div class="table">
                <el-table :key="tableKey" stripe v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
                    <el-table-column :label="$t('tables.measure.champs.champs1')" prop="id" sortable="custom" align="center" width="100" >
                        <template slot-scope="{row}"><span>{{ row.id }}</span></template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs2')" width="100" >
                        <template slot-scope="{row}">
                            <span >{{ row.position }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs3')" min-width="140px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs4')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.comment }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs5')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.type }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs6')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.losses }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs7')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.ts }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs8')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.measure1 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs9')" min-width="140px">
                        <template slot-scope="{row}">
                            <span >{{ row.ts1 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs10')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.measure2 }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs11')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.ts2 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs12')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.measure3 }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs13')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.ts3 }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs14')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.idProduct }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('tables.measure.champs.champs15')" min-width="100px">
                        <template slot-scope="{row}">
                            <span >{{ row.idFonction }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.measure.champs.champs16')" align="center" width="170" class-name="fixed-width">
