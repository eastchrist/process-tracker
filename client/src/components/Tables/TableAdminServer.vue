<template>
    <div class="TableAdminServer-container">
        <div class="wrapper">
            <div class="header">
                <h3>{{ $t('tables.server.title') }}</h3>
            </div>
            <div class="filter">
                <el-input v-model="listQuery.name" :placeholder="$t('tables.server.champs.champs4')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
            ...
            <div class="extra">
                <ExportToExcel className="ExportToExcel" TableName="Server" :TableList=list :defaultExcelHeater="listExcel" ></ExportToExcel>
                <ImportFromExcel className="ImportFromExcel" TableName="Server" :TableList=list :defaultExcelHeater="listExcel" :on-success="handleSuccess" :before-upload="beforeUpload"></ImportFromExcel>
                <UpdateFromNode className="UpdateFromNode" TableName="Server" @UpdateFromNode="getList"></UpdateFromNode>
            </div>
            <div class="table">
                <el-table ref="draggableTable" row-key="position" :key="tableKey" stripe v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
                    ...
                    <el-table-column :label="$t('tables.server.champs.champs4')" min-width="250px">
                        <template slot-scope="{row}">
                            <span class="link-type" @click="handleUpdate(row)">{{ row.name }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column :label="$t('tables.server.champs.champs5')" min-width="150px">
                        <template slot-scope="{row}">
                    ...
                    <el-table-column :label="$t('tables.server.champs.champs5')" align="center" width="170" class-name="fixed-width">
            ...
            <div class="treeview">
                <TreeAdminNodes class-name="TreeNodes_Server" :title="$t('tree.serverTree')" Find1="Server" Find2="" Find3=""></TreeAdminNodes>
            </div>
